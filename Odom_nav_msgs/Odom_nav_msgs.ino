...
nav_msgs::Odometry odom_msg;              //nav_message that is sent
...
// int for decimating the publishing rate
int i = 0;
...
{  
  ...
  //vx = U/2000*(dEr+dEl)/2*cos(theta)*1000/dt; //calculate vx value
  //vy = U/2000*(dEr+dEl)/2*sin(theta)*1000/dt; //calculate vy value
  //vth = U*(dEr-dEl)/(D*2000)/(dt);
  ...
  if (i >= 1999){
    ...
    //Serial.println(newEr);  
    i = 0;
  }
    nh.spinOnce();
  
  //delay(5); // delay is necessery as othewise the USB connenction gets troubles with the SPI communication
}
